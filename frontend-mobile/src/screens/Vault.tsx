...
function NoteCard({ n, onClick }: Any) {
  return (
    <div className="card card-pad col gap2" style={{ cursor: 'pointer', breakInside: 'avoid', marginBottom: 12 }} onClick={onClick}>
      ...
      <div className="row gap1 wrap">
        {n.tags.map((t: string) => <span key={t} className="t-xs tter mono">#{t}</span>)}
      </div>
...
function VaultScreen({ nav }: { nav: Nav }) {
  ...
  const v = NX.vault;
  const filtered = v.notes.filter((n: Any) => !q || n.title.toLowerCase().includes(q.toLowerCase()) || n.excerpt.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div className="col" style={{ height: '100%' }}>
      ...
      <div className="grow" style={{ overflowY: 'auto', padding: '0 16px 24px' }}>
        <ScreenHeader title="Vault" sub={`${v.notes.length} notas · ${v.folders.reduce((a: number, f: Any) => a + f.count, 0)} en total`} />
        ...
        {asking && (
          <div className="card card-pad col gap3 anim-up" style={{ marginBottom: 16, borderColor: 'var(--accent-soft-2)', background: 'linear-gradient(180deg, var(--accent-soft), transparent)' }}>
            ...
            <div className="col gap2">
              ...
              {v.rag.citations.map((c: Any) => (
                <div key={c.note} className="row gap2 t-sm card" style={{ padding: '8px 10px', cursor: 'pointer' }} onClick={() => nav.push('note', v.notes.find((n: Any) => n.title === c.note))}>
        ...
        {filtered.length ? (
          <div style={{ columnCount: 2, columnGap: 12 }}>
            {filtered.map((n: Any) => <NoteCard key={n.id} n={n} onClick={() => nav.push('note', n)} />)}
          </div>
        ) : (
...
// ---- Note editor (pushed) ----
function NoteScreen({ nav, data: n }: { nav: Nav; data: Any }) {
  ...
  return (
    <div className="col" style={{ height: '100%' }}>
      <TopBar left={<IconBtn name="arrow-left" onClick={() => nav.back()} />}
        title={n.folder}
        right={<><IconBtn name={mode === 'visual' ? 'square-pen' : 'eye'} onClick={() => setMode((m: string) => m === 'visual' ? 'code' : 'visual')} /><IconBtn name="more-horizontal" /></>} />
      <div className="grow" style={{ overflowY: 'auto', padding: '8px 20px 24px' }}>
        ...
        <div className="row gap2 wrap" style={{ marginBottom: 16 }}>
          {n.tags.map((t: string) => <span key={t} className="chip" style={{ height: 24 }}>#{t}</span>)}
        </div>
        {mode === 'visual' ? (
