...
function ProyectosScreen({ nav }) {
  const [filter, setFilter] = React.useState('Activos');
  const list = NX.projects.filter(p => p.status === filter);
  return (
      ...
      <div className="grow" style={{ overflowY: 'auto', padding: '0 16px 24px' }}>
        ...
        {list.length ? (
          <div className="col gap3 anim-screen" key={filter}>
            {list.map(p => <ProjectCard key={p.id} p={p} onClick={() => nav.push('project', p)} />)}
          </div>
        ) : (
...
function ProjectDetail({ nav, data: p }) {
          ...
          {NX.agents.map(a => (
            <div key={a.id} className="card card-pad col gap3" style={{ cursor: 'pointer' }} onClick={() => nav.push('agent', a)}>
              ...
              <div className="row gap1 wrap">
                {a.skills.map(s => <span key={s} className="chip" style={{ height: 24 }}>{s}</span>)}
              </div>
            </div>
          ))}
        </div>
...
function AgentDetail({ nav, data: a }) {
...
Object.assign(window, { ProyectosScreen, ProjectDetail, AgentesScreen, AgentDetail });
