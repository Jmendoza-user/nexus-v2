...
function Onboarding({ onDone, accent }) {
      ...
      <div className="grow anim-screen" key={step} style={{ overflowY: 'auto', padding: '8px 28px 16px' }}>
        ...
        {step === 'plan' && (
          <OnbBody title="Elige tu plan" sub="Puedes cambiarlo cuando quieras.">
            <div className="col gap3">
              {NX.plans.filter(p => p.id !== 'team').map(p => (
...
function PermRow({ icon, title, sub, on }) {
...
Object.assign(window, { LoginScreen, Onboarding, NexusMark, Dots });
