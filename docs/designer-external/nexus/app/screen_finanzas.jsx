...
// ============================================================
function SwipeCard({ children, onApprove, onReject, onTap }) {
  const [dx, setDx] = React.useState(0);
  const [drag, setDrag] = React.useState(false);
  const [leaving, setLeaving] = React.useState(0); // -1 left, 1 right
  const startX = React.useRef(0);
  const moved = React.useRef(false);
  const TH = 92;

  function down(e) { setDrag(true); moved.current = false; startX.current = (e.touches ? e.touches[0].clientX : e.clientX); }
  function move(e) {
    if (!drag) return;
...
function FinanzasInbox({ nav }) {
  const [drafts, setDrafts] = React.useState(NX.drafts);
...
function DraftDetail({ nav, data: d }) {
...
function RecurringToggle() { const [on, setOn] = React.useState(false); return <Toggle on={on} onChange={setOn} />; }

Object.assign(window, { FinanzasScreen, DraftDetail });
