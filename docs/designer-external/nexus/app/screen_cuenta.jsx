    ...
    <div className="col" style={{ height: '100%' }}>
      ...
      <div className="grow" style={{ overflowY: 'auto', padding: '0 16px 24px' }}>
        ...
        <div className="card card-pad col gap4" style={{ marginBottom: 20 }}>
          ...
          {NX.usage.map(u => <QuotaRow key={u.label} {...u} />)}
        </div>
...
// ---- Asistente principal ----
function ConfigPrincipal({ nav }) {
  const [voice, setVoice] = React.useState('Elisa María');
  const [proact, setProact] = React.useState(3);
  return (
...
// ---- Preferencias ----
function ConfigPreferencias({ nav, theme, setTheme }) {
...
// Shared config shell
const cfgLbl = { textTransform: 'uppercase', letterSpacing: '0.06em', margin: '4px 4px 10px' };
function ConfigShell({ nav, title, children }) {
  return (
    <div className="col" style={{ height: '100%' }}>
...
Object.assign(window, { CuentaScreen, ConfigPrincipal, ConfigSkills, ConfigConexiones, ConfigSeguridad, ConfigPreferencias, UsoScreen });
